...
本設計は単一ユーザー運用を前提に、欠損のある入力でもドラフト保存を許容し、必要時に評価や比較を LLM に委譲できる構成とする。統合オーケストレーターは持たず、CLI または外部スクリプトが各エージェントを個別に実行してフローを構成する。求人・会社情報の補完は許可済み情報源のみに限定し、クローリングは禁止とする。
...
|-------------|---------|------------|------------|-------|
| 1.1, 1.2, 1.3, 1.4 | 実行コンテキスト構築 | CLI, SessionBuilder, WorkflowBuilder | ExecutionContext, SessionBuilderService | プロフィール/求人/評価 |
...
|-----------|--------------|--------|--------------|--------------------------|-----------|
| CLI Commands | CLI | 入力受付と実行開始 | 1.1, 1.2 | Typer (P0) | Service |
| SessionBuilder | セッション | 入力正規化と実行コンテキスト生成 | 1.1, 1.2 | なし | Service, State |
...
- ProfileDraft / ProfileResult: 欠損情報を含むプロフィール成果物
